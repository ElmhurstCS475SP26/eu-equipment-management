...
function CatalogFiltersAndGrid({ initialEquipment, linkPrefix }) {
  ...
  const categories = Array.from(new Set(initialEquipment.map((item) => item.category)));
  const availabilityOptions = ["Available", "Reserved", "Maintenance"];

  const filteredEquipment = initialEquipment.filter((item) => {
    const matchesSearch =
...
export default function CatalogClient({ initialEquipment, linkPrefix = "/equipment" }) {
  return (
    <Suspense fallback={<div className="p-6">Loading filters...</div>}>
      <CatalogFiltersAndGrid initialEquipment={initialEquipment} linkPrefix={linkPrefix} />
    </Suspense>
  );
}
